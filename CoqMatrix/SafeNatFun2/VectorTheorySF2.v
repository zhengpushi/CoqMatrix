...
Require Export SafeNatFun2.vec.
...
Module BasicVectorTheorySF (E : ElementType).
  ...
  (** mapping of a vector *)
  Definition vmap {n} (v : vec n) f : vec n := vmap f v.
  
  (** folding of a vector *)
  (* Definition vfold : forall {B : Type} {n} (v : vec n) (f : A -> B) (b : B), B. *)
  
  (** mapping of two matrices *)
  Definition vmap2 {n} (v1 v2 : vec n) f : vec n := vmap2 f v1 v2.
...
Module RingVectorTheorySF (E : RingElementType) <: RingVectorTheory E.
  ...
  (** ** Zero vector *)
  Definition vec0 {n} : vec n := vec0 Azero n.
